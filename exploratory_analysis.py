...
for combo in fs_combos:
    ...
    if 'scores' in combo:
        ...
        for prefix in consts.SCORE_PREFIXES:
            ...
            if i == 0:
                for epoch in ['study_day', 'study_month']:
                    df2[epoch] = 0
            if i == 1: # Prefix B_, the 4-month surveys; predict into the past
                df2['study_day'] = 119
                df2['study_month'] = 3
...
for horizon in consts.TARGET_HORIZONS:
    ...
    if horizon == 'study_day':
        df2 = horizons_df[groupby_cols + ['is_weekday']]
        df = df.merge(df2, on=groupby_cols)
#         print(df[df.duplicated(groupby_cols)])
    else:
    ...
    # Add back in the other horizons - might need these later
    merge_cols = [dataset.id_col] + consts.TARGET_HORIZONS
    df = df.merge(horizons_df[merge_cols],
                  on = groupby_cols)
...
for s_feats in static_featuresets:
    for t_feats in temporal_featuresets:
        ...
        if s_feats.name != 'scores' or t_feats.horizon != 'study_week':
            ...
            ''' We only want to retain the horizon we are interested in.
                Let's drop other horizon columns from each dataframe.
                Make sure we don't modify the original dataframes!
            '''
            s_df = s_feats.df.drop(columns=[col for col in s_feats.df.columns 
                                            if col in consts.TARGET_HORIZONS 
                                            and col != t_feats.horizon])
            
            t_df = t_feats.df.drop(columns=[col for col in t_feats.df.columns 
                                            if col in consts.TARGET_HORIZONS 
                                            and col != t_feats.horizon])
            
            df = s_df.merge(t_df, on=[s_feats.id_col, t_feats.horizon])
...
for t_feats in temporal_featuresets:
    ...
    t_feats.df = t_feats.df[t_feats.df['study_month'] > 0]
    t_feats.df.drop(columns=[col for col in t_feats.df.columns 
                             if col in consts.TARGET_HORIZONS 
                             and col != t_feats.horizon],
                    inplace=True
                   )
...
#     res = models.predict(t_feats_lagged, n_lags, classifiers=['LogisticR', 'RF', 'XGB'], optimize=True)
#     print(res)
#     all_results.append(res)       
